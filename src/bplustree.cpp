...
void BPlusTree::InsertKey(std::string key, std::shared_ptr<Block> blockPtr) {
    ...
    if (root == nullptr) {
        ...
        if (traverseNode->keys.size() < size) {
            ...
            if (duplicate && insertPos != -1) {
                ...
                std::vector<std::string>::iterator keyInsertItr = traverseNode->keys.begin() + insertPos;
            ...
            std::vector<std::string> tempKeys(traverseNode->keys);
            ...
            if (duplicate && insertPos != -1) {
                ...
                std::vector<std::string>::iterator insertItr = tempKeys.begin() + insertPos;
            ...
            std::vector<std::string> firstHalfKey(tempKeys.cbegin(), tempKeys.cbegin() + splitSize);
            std::vector<std::string> secondHalfKey(tempKeys.cbegin() + splitSize, tempKeys.cend());
...
void BPlusTree::InsertInternal(std::string key, std::shared_ptr<Node> parent, std::shared_ptr<Node> child) {
    ...
    if (parent->keys.size() < size) {
        ...
        if (insertPos != -1) {
            // insert key in vector
            std::vector<std::string>::iterator keyInsertItr = parent->keys.begin() + insertPos;
        ...
        std::vector<std::string> tempKeys(parent->keys);
        ...
        if (insertPos != -1) {
            // insert key in vector
            std::vector<std::string>::iterator keyInsertItr = tempKeys.begin() + insertPos;
        ...
        std::size_t splitSize = (tempKeys.size() + 1)/2;
        std::vector<std::string> firstHalfKey(tempKeys.cbegin(), tempKeys.cbegin() + splitSize);
        std::vector<std::string> secondHalfKey(tempKeys.cbegin() + splitSize + 1, tempKeys.cend());    // +1 becuase for the second half dont need the first key
        ...
        std::string newParentKey = tempKeys[splitSize];
        numNodes++;
...
void BPlusTree::PrintRecordOfKey(std::shared_ptr<void> ptr, std::string key) {
...
std::vector<std::pair<std::string, std::shared_ptr<std::vector<std::shared_ptr<Block>>>>> BPlusTree::FindRange(std::string begin, std::string end) {
    std::vector<std::pair<std::string, std::shared_ptr<std::vector<std::shared_ptr<Block>>>>> blkPtrs;
    ...
    while (!terminate) {
        ...
        for (int i = 0; i < keySize; i++) {
            if (itr->keys[i] > end) {
                ...
                std::pair<std::string, std::shared_ptr<std::vector<std::shared_ptr<Block>>>> newPair = std::make_pair(
                    itr->keys[i],
...
int BPlusTree::DeleteKey(std::string key) {
    ...
    if (root == nullptr) {
        ...
        if (leftSibling >= 0) {
            ...
            if (leftSiblingNode->keys.size() >= ((size + 1)/2) + 1) {
                ...
                std::string key = leftSiblingNode->keys[leftSiblingNode->keys.size() - 1];
...
int BPlusTree::RemoveInternal(std::string key, std::shared_ptr<Node> traverseNode, std::shared_ptr<Node> childToDelete) {
