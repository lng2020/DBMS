...
{
	// 创建根目录DBMS
	// char* command = "IF NOT EXIST ./DBMS (mkdir .\\DBMS)";
	// system(command);
	// // 判断新数据库名是否已经存在
	// char*  path = "./DBMS/" + dbroot->name + "/";
...
void createTable(char *tableval, struct hyper_items_def *Hitemroot)
{
	...
	struct hyper_items_def *fileitemstemp = Hitemroot;
	if (dbroot != NULL)
		dbtemp = dbroot;
	else
	{
	...
	//写文件228-287
	string databaseName = dbtemp->name;
	string tableName = tableval;
	...
	//获取属性
	i = 1;
	while (fileitemstemp != NULL && i < 10)
	{
		Field temfield;
		temfield.FieldName = fileitemstemp->field;
		if (fileitemstemp->type == 0)
			temfield.FieldType = "INT";
		else
			temfield.FieldType = "VARCHAR";
		//默认第一个是主键
		if(i == 1)
			temfield.Key = 1;
		else temfield.Key = 0;
		temfield.NullFlag = 0;
		temfield.Size = 255;
		temfield.ValidFlag = 1;
		i++;
		
		fields.push_back(temfield);
	}
	// 创建新表
	...
	for (int i = 0; i < fields.size(); i++)
	{
		cout << fields[i].FieldName << "," << fields[i].FieldType << "," << fields[i].Size << "," << fields[i].Key << "," << fields[i].NullFlag << "," << fields[i].ValidFlag << endl;
...
bool whereTF(int i, struct table *tabletemp, struct conditions_def *conroot)
{
	if (conroot == NULL)
	{
		return TRUE;
